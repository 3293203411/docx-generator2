...
def merge_runs_in_paragraph(paragraph: Paragraph) -> dict:
    """
    合并段落中所有run的文本，返回合并后的文本和run信息
    用于处理Word将文本拆分为多个run的情况
    """
...
def merge_runs_in_cell(cell) -> dict:
    """合并表格单元格中所有run的文本"""
    full_text = ""
    all_runs = []
    ...
    for paragraph in cell.paragraphs:
        ...
        full_text += para_info["text"]
        all_runs.extend(para_info["runs"])
        if not first_formatting and para_info["formatting"]:
            first_formatting = para_info["formatting"]
    
    return {
        "text": full_text,
        "runs": all_runs,
        "formatting": first_formatting
    }
...
async def generate_document(request: GenerateRequest):
    ...
    - text_keys: 占位符名称数组，如 ["公司名称", "项目名称"]
    - text_values: 替换内容数组，如 ["XX公司", "XX项目"]
    - filename: 输出文件名（可选）
    """
    try:
        # 解析参数
